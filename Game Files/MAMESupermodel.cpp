...
static void FFBGameEffects(EffectConstants* constants, Helpers* helpers, EffectTriggers* triggers, int stateFFB, LPCSTR name)
{
	if (RunningFFB == NamcoFFBNew) // Ace Driver
	{
		...
		// -----------------------------------------
		// Native wheel motor output (Ace Driver)
		// -----------------------------------------
		if (name == wheel_motor)
		{
			// SIGNED force: -62 .. 0 .. +62
			...
			// -----------------------------------------
			// Force FROM LEFT  (positive values)
			// -----------------------------------------
			if (fface > 0)
			{
			...
			// -----------------------------------------
			// Force FROM RIGHT (negative values)
			// -----------------------------------------
			else if (fface < 0)
			{
				double percentForce = (-fface) / 62.0;
			...
			// -----------------------------------------
			// True neutral (only when raw == 0xFF)
			// -----------------------------------------
			else
			{
